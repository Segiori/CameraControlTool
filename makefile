...
GTKMMOPTION = `pkg-config --cflags --libs gtkmm-3.0`
#
...
%.o : %.cpp
	$(CC) -I. $(INC_PATH) $(CXX_COMPILE_OPTIONS) $(COMMON_OPTIONS) $(ARCH_OPTIONS) $(GTKMMOPTION) -c $< -o $@

%.o : %.c
	$(CC) -I. $(INC_PATH) $(C_COMPILE_OPTIONS) $(COMMON_OPTIONS) $(ARCH_OPTIONS) $(GTKMMOPTION) -c $< -o $@
